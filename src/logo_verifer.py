...
def method_one(image, descriptores):
    ...
    for brand, logo_data in descriptores.items():
        for idx, (kp_logo, des_logo) in enumerate(logo_data):
            ...
            if len(good) >= MIN_MATCH_COUNT:
                src_pts = np.float32([kp_logo[m.queryIdx].pt for m in good]).reshape(
                    -1, 1, 2
                )
                dst_pts = np.float32([kp_cloth[m.trainIdx].pt for m in good]).reshape(
                    -1, 1, 2
                )
    ...
    if best_match_count < BRAND_CONFIDENCE_THRESHOLD:
        print(
            f"RESULT: UNKNOWN BRAND (only {best_match_count} matches found, threshold is {BRAND_CONFIDENCE_THRESHOLD})"
        )
...
def method_two(image, descriptores):
    def extract_logo_regions(image):
        ...
        dilated = morphological_operations.dilate(
            edges, iterations=2, kernel_size=(5, 5)
        )
    ...
    for region_idx, (x, y, w, h, roi) in enumerate(logo_regions):
        ...
        for brand, logo_data in descriptores.items():
            for idx, (kp_logo, des_logo) in enumerate(logo_data):
                ...
                if len(good) >= MIN_MATCH_COUNT:
                    src_pts = np.float32(
                        [kp_logo[m.queryIdx].pt for m in good]
                    ).reshape(-1, 1, 2)
                    dst_pts = np.float32([kp_roi[m.trainIdx].pt for m in good]).reshape(
                        -1, 1, 2
                    )
